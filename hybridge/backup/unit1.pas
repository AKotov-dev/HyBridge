...
var
  Conf: TStringList;
  SPEED_UP, SPEED_DOWN, OBFS_TYPE: string;
begin
  ...
  try
    ...
    if SpeedAuto.Checked then
    begin
      SPEED_UP := SpeedUpEdit.Text;
      SPEED_DOWN := SpeedDownEdit.Text;
    end;
    //Тип обфускатора
    case RadioGroup1.ItemIndex of
      0: OBFS_TYPE := 'salamander';
      1: OBFS_TYPE := 'gecko';
    end;

    Memo1.Text := BuildHysteria2URI(EditServerIP.Text, EditUDPPort.Text,
      AUTH_PASS, OBFS_TYPE, OBFS_PASS, True, SPEED_UP, SPEED_DOWN, 'HyBridge');
...
var
  AUTH_PASS, OBFS_PASS, OBFS_TYPE, config: string;
begin
  ...
  begin
    ...
    OBFS_TYPE := GetJSONValue(config, 'outbounds[0].obfs.type');
    case OBFS_TYPE of
      'salamander': RadioGroup1.ItemIndex := 0;
      'gecko': RadioGroup1.ItemIndex := 1;
      else
    ...
    //Получаем URI
    Memo1.Text := BuildHysteria2URI(EditServerIP.Text, EditUDPPort.Text,
      AUTH_PASS, OBFS_TYPE, OBFS_PASS, True,
      // Boolean, без кавычек
...
begin
  ...
  try
    ...
    Conf.Add('');
    Conf.Add('quic:');
    //YouTube: "...повторите подключение позже; Default=30s"
